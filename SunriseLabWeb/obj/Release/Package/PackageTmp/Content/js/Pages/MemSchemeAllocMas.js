...
var AddNew = function () {
    window.location.href = '/Master/MemSchemeAllocDet';
}
...
function UserDetailPage(params) {
    return '<a onclick="EditData(\'' + params.data.Customer_Id + '\')" ><i class="fa fa-pencil-square-o" aria-hidden="true" style="font-size: 17px;cursor:pointer;"></i></a>'
        + '&nbsp;&nbsp;<a onclick="DeleteUserDetail(\'' + params.data.Customer_Id + '\',\'' + params.data.MemberId + '\')" ><i class="fa fa-trash-o" aria-hidden="true" style="font-size: 17px;cursor:pointer;"></i></a>';
}
function EditData(Customer_Id) {
    location.href = '/Master/MemSchemeAllocDet?C=' + Customer_Id;
}
    ...
    $.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
            ...
            if (data.Status == "0") {
                ...
                ClearRemoveModel();
                toastr.success("Scheme Allocation Detail Delete Successfully");
                GetDataList();
            }
        },
...
const datasource1 = {
    getRows(params) {
        ...
        $.ajax({
            url: "/Master/MemberWise_SchemeAllocation_Select",
            async: false,
            type: "POST",
            data: formData,
            processData: false,
